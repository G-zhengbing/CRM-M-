  ...
  <div class="shade">
    <Card>
      ...
      <p class="contanner">
        ...
        <FormItem label="缩略图" prop="event_pic">
...
export default {
  ...
  methods: {
    ...mapActions(["addSpecialList","updataSpecialLIst"]),
		catchData(val){
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          if(this.$parent.isUpdata){
            this.updataSpecialLIst(this.form).then(res=>{
              console.log(res)
              this.$Message.success('修改成功');
            })
          }else{
            this.addSpecialList(this.form).then(res=>{
              console.log(res)
